...
const UpdateInfo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const formResponse = await axios.get(`http://localhost:8703/api/formdata/${id}`);
        setData(formResponse.data); // Update form data

        const skillResponse = await axios.get('http://localhost:8703/api/skills');
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            {userSkill?.profilePicture && (
              <img
                src={`http://localhost:8703${userSkill.profilePicture}`} // Show the picture if exists
                alt="Profile"
